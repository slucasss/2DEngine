...
CircleTest::CircleTest(Vector2f coor) : Entity(coor), Movable(Vector2f(0, 0), Vector2f(200,0)){
...
void CircleTest::Draw(Window* window){
...
void CircleTest::update(float deltaTime){
	if (InputManager::GetInstance()->IsDown("n")) {
		SceneManager::GetInstance()->SetCurrentScene(3);
	}
	...
	if (GetCoordinates().GetY() + 25 > 500) {
		PhysiqueManager::GetInstance()->Bounce(this, Vector2f(0, 1), 0.75);	
		SetCoordinates(Vector2f(GetCoordinates().GetX(), 500 - 25));
	}
	if (GetCoordinates().GetX() + 25 > 1000) {
		PhysiqueManager::GetInstance()->Bounce(this, Vector2f(1, 0), 0.75);
